...
uint32_t* internal_node_child(void* node, uint32_t child_num)
{
    ...
    if (child_num > num_keys)
    {
        ...
        /*
        For an internal node, the maximum is always the right child
        */
...
void initialize_internal_node(void* node)
{
    ...
    *internal_node_num_keys(node) = 0;
    /*
    Necessary because the root page number is 0; by not initializing an
    internal node's right child to an invalid page number when initializing the node, we may
...
void create_new_root(Table* table, uint32_t right_child_page_num)
{
   ...
   /* Root node is a new internal node with one key and 2 children */
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num)
{
   ...
   */
   ...
   /*
   If we are already at max number of cells for a node, we cannot increment
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num)
{
    ...
    the step where the table's new root is created. If it does not,
    we have to insert the newly created node into its parent
  ...
  */
  ...
  /*
  Set child before middle key, which is now the highest key, to the node's right child,
 ...
 *internal_node_right_child(old_node) = *internal_node_child(old_node, *old_num_keys - 1);
 ...
 /*
 Determine which of the 2 nodes after the split should contain the child to be inserted,
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value)
{
   ...
   *leaf_node_next_leaf(old_node) = new_page_num;
   ...
   /*
   All existing keys plus new should be divided
